Shader "Nature/OceanAdvanced2"
{
	Properties
	{
		...
		_BaseColorAlpha ("Water Opacity", Range (0, 1))        = 1
		_Vertex_NormalF ("Vertex Normal Factor", Range (0, 1)) = 1
	...
	// Superpose the regular waves to get irregular(shor-crested/long-crested) wave
	...
	// Normal map of deformed water surface to get the ligtht(sun) reflection right
	...
	// if detailed_normal is set to zero, the wave look quite smooth
	...
	half4 frag( v2f i ) : SV_Target
	{
		...
		half3 normal        = normalize(detail_normal * 1.0 + vertex_normal * _Vertex_NormalF); // ori = 0.0
		...
		// baseColor.rgb = getSeaColor(i.world_position, normal, normalize(light_direction), normalize(eye_vector), eye_vector);
			...
			/* SSR test */
				...
				// float3 reflected = tex2Dproj(_RefractionTex, UNITY_PROJ_COORD(pixelPosition)) * _ReflectionColor;  // Originally cmt out
			#endif
	...
	Subshader
	{
		...
		// Tags {"RenderType"="Opaque" "Queue"="Opaque"} // by MYO
		Lod 300
		// ColorMask RGBA
